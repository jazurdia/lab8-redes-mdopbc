...
# Asegurarse de que el número de bytes sea múltiplo de 16 (para AES)
padding_length = 16 - (len(image_bytes) % 16)
padded_image_bytes = image_bytes + bytes([padding_length]) * padding_length

# Cifrar los bytes usando AES en modo CBC con un vector de inicialización (IV)
key = os.urandom(16)  # Generar una clave AES de 128 bits (16 bytes)
iv = os.urandom(16)   # Generar un vector de inicialización (IV) de 16 bytes
